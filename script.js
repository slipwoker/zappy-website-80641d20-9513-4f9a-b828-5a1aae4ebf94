document.addEventListener('DOMContentLoaded', function() {
    ...
    if (contactForm) {
        ...
        function validateField(input) {
            ...
            if ((type === 'email' || name.includes('email')) && value !== '') {
    ...
    if (animatedElements.length > 0) {
        ...
        style.textContent = `
            .fade-in.visible, .fade-in.animated { opacity: 1 !important; }
            .fade-in-up { transform: translateY(30px); }
            .fade-in-up.visible, .fade-in-up.animated { opacity: 1 !important; transform: translateY(0) !important; }
            .fade-in-left { transform: translateX(-30px); }
            .fade-in-left.visible, .fade-in-left.animated { opacity: 1 !important; transform: translateX(0) !important; }
            .fade-in-right { transform: translateX(30px); }
            .fade-in-right.visible, .fade-in-right.animated { opacity: 1 !important; transform: translateX(0) !important; }
            [dir="rtl"] .fade-in-left { transform: translateX(30px); }
            [dir="rtl"] .fade-in-left.visible, [dir="rtl"] .fade-in-left.animated { transform: translateX(0) !important; }
            [dir="rtl"] .fade-in-right { transform: translateX(-30px); }
            [dir="rtl"] .fade-in-right.visible, [dir="rtl"] .fade-in-right.animated { transform: translateX(0) !important; }
        `;
...
/* ZAPPY_HERO_3D_RUNTIME (Design Personality Engine) */
(function(){
  if (typeof window === 'undefined' || typeof document === 'undefined') return;
  if (window.__zappyHero3dRuntimeInit) return;
  window.__zappyHero3dRuntimeInit = true;

  var HERO_3D_ENABLED = true;
  ...
  function hideFallback(fallback){
    ...
    if (fallback.style && fallback.style.setProperty) {
  ...
  function revealFallbacks(){
    document.documentElement.classList.add('hero-3d-reduced');
    q('[data-hero-3d-model]').forEach(function(host){
      host.setAttribute('data-hero-3d-fallback-visible', 'true');
      var fallback = host.querySelector('[data-hero-3d-fallback], img, picture, video');
  ...
  function mountViewer(host, index){
    if (!host || host.__zappyHero3dMounted) return;
    host.__zappyHero3dMounted = true;
    var src = normalizeAssetPath(host.getAttribute('data-model-src') || host.getAttribute('data-hero-3d-src'));
    if (!src) {
      host.setAttribute('data-hero-3d-fallback-visible', 'true');
      return;
    }

    var poster = normalizeAssetPath(host.getAttribute('data-model-poster') || host.getAttribute('data-hero-3d-poster'));
    var backgroundSrc = normalizeAssetPath(host.getAttribute('data-model-background-src') || '');
    var background = null;
    if (backgroundSrc) {
      background = document.createElement('img');
      background.className = 'zappy-hero-3d-background';
      background.src = backgroundSrc;
      background.alt = '';
      background.loading = index === 0 ? 'eager' : 'lazy';
      background.decoding = 'async';
      background.setAttribute('aria-hidden', 'true');
      host.setAttribute('data-hero-3d-composite', 'true');
      host.insertBefore(background, host.firstChild);
    }

    var viewer = document.createElement('model-viewer');
    viewer.className = 'zappy-hero-3d-viewer';
    viewer.setAttribute('src', src);
    if (poster) viewer.setAttribute('poster', poster);
    viewer.setAttribute('loading', index === 0 ? 'eager' : 'lazy');
    viewer.setAttribute('reveal', 'auto');
    viewer.setAttribute('auto-rotate', '');
    viewer.setAttribute('rotation-per-second', host.getAttribute('data-model-rotation-speed') || '28deg');
    viewer.setAttribute('camera-orbit', host.getAttribute('data-model-camera-orbit') || '0deg 72deg 105%');
    viewer.setAttribute('field-of-view', host.getAttribute('data-model-field-of-view') || '32deg');
    viewer.setAttribute('shadow-intensity', host.getAttribute('data-model-shadow-intensity') || '0.8');
    viewer.setAttribute('exposure', host.getAttribute('data-model-exposure') || '1');
    viewer.setAttribute('interaction-prompt', 'none');
    viewer.setAttribute('disable-zoom', '');
    viewer.setAttribute('touch-action', 'pan-y');
    viewer.setAttribute('aria-label', host.getAttribute('data-model-label') || 'Interactive 3D product model');

    if (host.hasAttribute('data-model-controls')) viewer.setAttribute('camera-controls', '');

    var fallback = host.querySelector('[data-hero-3d-fallback], img, picture, video');
    if (fallback) fallback.setAttribute('data-hero-3d-fallback', '');
    host.insertBefore(viewer, background ? background.nextSibling : host.firstChild);

    viewer.addEventListener('load', function(){
      host.setAttribute('data-hero-3d-loaded', 'true');
      host.removeAttribute('data-hero-3d-fallback-visible');
      hideFallback(fallback);
      if (window.gsap) {
        window.gsap.fromTo(viewer, { y: 28, scale: 0.94, opacity: 0 }, { y: 0, scale: 1, opacity: 1, duration: 0.9, ease: 'power3.out', overwrite: true });
      }
    });
    viewer.addEventListener('error', function(){
      host.removeAttribute('data-hero-3d-loaded');
      host.setAttribute('data-hero-3d-fallback-visible', 'true');
      showFallback(fallback);
    });
    ...
    if (window.gsap && window.ScrollTrigger) {
      window.gsap.to(viewer, {
        ...
        rotateZ: 1.8,
        ease: 'none',
        scrollTrigger: { trigger: host.closest('[data-cinematic-hero], section') || host, start: 'top top', end: 'bottom top', scrub: true }
      });
    }
  }
  ...
  ready(function(){
    ...
    var hosts = q('[data-hero-3d-model]');
    if (!hosts.length) return;
    if (reduce) { revealFallbacks(); return; }
    loadModelViewer(function(err){
      if (err || !(window.customElements && window.customElements.whenDefined)) {
        revealFallbacks();
        return;
      }
      window.customElements.whenDefined('model-viewer').then(function(){
        hosts.forEach(mountViewer);
      }).catch(revealFallbacks);
    });
  });
})();
/* End ZAPPY_HERO_3D_RUNTIME */


/* SIGNATURE MOMENT: gradient-follow */
(function(){

      (function(){
        var layer = document.createElement('div'); layer.className = 'zsm-gradient-follow'; document.body.insertBefore(layer, document.body.firstChild);
        if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
        if (window.matchMedia && !window.matchMedia('(pointer: fine)').matches) return;
        document.addEventListener('mousemove', function(e){
          layer.style.setProperty('--zsm-gx', (e.clientX / window.innerWidth * 100).toFixed(1) + '%');
          layer.style.setProperty('--zsm-gy', (e.clientY / window.innerHeight * 100).toFixed(1) + '%');
        }, { passive: true });
      })();
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '80641d20-9513-4f9a-b828-5a1aae4ebf94',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '80641d20-9513-4f9a-b828-5a1aae4ebf94';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '80641d20-9513-4f9a-b828-5a1aae4ebf94';
